...
- Python：需要 Python 3.10 虚拟环境（系统 `/usr/bin/python3` 是 3.8，import `chat` 会失败）
  - 如有 AIGC-SDK 可用 `/data/AIGC-SDK/hub_venv`；否则参考 README 创建 Python 3.10 虚拟环境
- 依赖：`flask`、`transformers`（仅用 tokenizer）

## 关键坑点

### 1. Python 版本必须 3.10
`chat.cpython-310-aarch64-linux-gnu.so` 是 cpython 3.10 ABI 编译产物。用 3.8 启动会报 `No module named 'chat'`（虽然 `.so` 文件就在 sys.path 里）。**必须用 Python 3.10 虚拟环境启动**。
...
gssh exec "cd /data/LLM-TPU/models/Qwen3/python_demo && \
  setsid $VENV/bin/python3.10 qwen3_web.py \
